                    ...
                    if user != message.author:
                        db.update_one(
                            {"id": user.id},
                            {
                                "$inc": {"rep": 1},
                                "$push": {
                                    "history": {"date": message.created_at, "rep": 1}
                                },
                            },
                            upsert=True,
                        )
                        embed = discord.Embed(
                            description=f"Gave +1 rep to {user.display_name}"
                        )
                        await message.channel.send(embed=embed)

    @discord.slash_command(
        name="repgraph", description="Displays a user's reputation over time"
    )
    async def repgraph(self, ctx: discord.Interaction, user: discord.User):
        ...
        if not data or "history" not in data:
            await ctx.edit_original_message(
                content=f"No reputation data found for {user.display_name}"
            )
            return
        ...
        fig = px.line(
            df, x="date", y="rep", title=f"{user.display_name}'s Reputation Over Time"
        )
